var muctieu_tk, lat_do, lat_phut, long_do, long_phut; 
var h_gio, v_gio, h_dong, v_dong, phuongtien_hh;
var t, v_tk,ss;
var v_td, h_qs, lantk;
var chiso_lat,chiso_long, phuongtien_hh1;
var outputText;
        
        function validate() {
            // get the input
            muctieu_tk = document.forms["input_form"]["muctieu_tkterm"].value;//Loại mục tiêu tìm kiếm
            phuongtien_hh = document.forms["input_form"]["phuongtien_hhterm"].value;// Phương tiện hàng hải của tàu bị nạn
            phuongtien_hh1 = document.forms["input_form"]["phuongtien_hh1term"].value;// Phương tiện hàng hải của tàu tìm kiếm
            ...
            lantk = document.forms["input_form"]["lantkterm"].value;//tầm nhìn tại hiện trường (km)
            ss = document.forms["input_form"]["ssterm"].value;//sai số trôi dạt
            t = document.forms["input_form"]["tterm"].value;//thời gian trôi dạt

            if (lat_do >90 || lat_do <0){
            ...
            } else if (lat_phut >60 || lat_phut <0){
            ...
            } else if (long_phut >60 || long_phut <0){
            ...
            //tính toán giá trị
            var s_dongchay= v_dong*t;//độ dạt do dòng chảy sau t
            var v_dongdogio1 = 0.036*v_gio;// vận tốc dạt dòng do gió
            var b = parseFloat(v_dongdogio1); var v_dongdogio = Math.round(b * 1000)/1000;
            var s_dongdogio1 = 0.036*v_gio*t;// độ dạt dòng do gió sau t
            var c = parseFloat(s_dongdogio1); var s_dongdogio = Math.round(c * 1000)/1000;
            var lw_trai1 = h_gio_nguoc/1 - goclech;// hướng Lw trái
            var lw_phai1 = h_gio_nguoc/1 + goclech;// hướng LW phải
            var v_datgio = k * v_gio + hc/1;// vận tốc dạt do gió
            var s_datgio1 = t*(k * v_gio + hc/1);//độ dạt do gió
            var a = parseFloat(s_datgio1); var s_datgio = Math.round(a * 1000)/1000;// làm tròn s_datgio1
            ...
            //tính TWC
                var x_TWC = x1 + x2;
                var y_TWC = y1 + y2;
                var v_TWC = Math.sqrt(Math.pow(x_TWC,2)+Math.pow(y_TWC,2));//chưa nhân thời gian t
                var s_TWC = v_TWC*t;
            //tính Dt (_Dt) và dt
                var x_Dt = x_TWC + x3;
                var y_Dt = y_TWC + y3;
            ...
            var v_LW = v_datgio*Math.cos(goclech*3.14/180);// tính v_LW
            var s_LW = v_datgio*t;// tính độ dạt theo LW
            ...
            outputText_dongdogio = h_dongdogio + " độ; "+ v_dongdogio + " M/h";
            outputText_troidatdongdogio = h_dongdogio + " độ; " + t + " x " + v_dongdogio  + " = " + s_dongdogio + " M";
            outputText_goclech = "±" + goclech + " độ";
            outputText_LWtrai = lw_trai + " độ";
            outputText_LWphai = lw_phai + " độ"; 
            outputText_v_datgio = k + " x " + v_gio + " + " + hc + " = "+ v_datgio1 + " M/h";
            outputText_s_datgio = s_datgio + " M";
            outputText_kc_trai = _Dt1*t + " M";
            outputText_kc_phai = _Dp1*t + " M";
            outputText_dt = dt1 + " M";
            outputText_dp = dp1 + " M";
